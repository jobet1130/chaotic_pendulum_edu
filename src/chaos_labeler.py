...
class ChaosLabeler:
    ...
    def calculate_lyapunov_exponent(self, time_series: np.ndarray, 
                                    ...
                                    time_delay: Optional[int] = None) -> float:
        ...
        for i in range(max_steps):
            ...
            d_i = np.linalg.norm(vectors[valid_indices + i] - vectors[evolved_indices], axis=1)
    ...
    def detect_chaos(self, df: pd.DataFrame, column: str = "num__theta", 
                     use_correlation_dim: bool = True) -> pd.DataFrame:
        ...
        for drive_force, group in grouped:
            ...
            if use_correlation_dim:
                ...
                logging.info(f"Correlation dimension for drive force {drive_force:.4f}: {corr_dim:.4f}")
            ...
            lyapunov_threshold = self.config["chaos_detection"].get("threshold", 0.1) * 0.5
            corr_dim_threshold = 1.5  # Chaotic systems often have correlation dimension > 1.5
            
            # System is chaotic if either indicator suggests chaos
            is_chaotic_lyapunov = lyapunov > lyapunov_threshold
            is_chaotic_corr_dim = corr_dim > corr_dim_threshold if use_correlation_dim else False
            ...
            results.append({
                ...
                "is_chaotic_corr_dim": is_chaotic_corr_dim if use_correlation_dim else None
            })
            
            logging.info(f"Drive force {drive_force:.4f}: Lyapunov={lyapunov:.6f}, "
                        f"Corr. Dim={corr_dim:.4f}, Chaotic={is_chaotic}")
        
        results_df = pd.DataFrame(results)
        print(f"✅ Chaos detection complete. Found {results_df['is_chaotic'].sum()} chaotic cases.")
        
        # Merge results with original data
        labeled_df = df.merge(results_df[["drive_force_c", "is_chaotic"]], 
    ...
    def calculate_correlation_dimension(self, time_series: np.ndarray,
                                       embedding_dim: Optional[int] = None,
        ...
        N = len(time_series)
        if N < (embedding_dim - 1) * time_delay + 100:
            logging.warning("Time series too short for reliable correlation dimension estimation")
            return 0.0
        ...
        if np.sum(valid_indices) < 3:
            logging.warning("Not enough valid points for correlation dimension estimation")
            return 0.0
    ...
    def plot_phase_space(self, df: pd.DataFrame, drive_forces: Optional[List[float]] = None,
                        save_dir: Optional[Union[str, Path]] = None,
                        plot_3d: bool = True) -> None:
        ...
        if drive_forces is None:
            ...
            if "is_chaotic" in df.columns:
                ...
                non_chaotic = df[df["is_chaotic"] == False]["num__drive_force_c"].unique()
                
                chaotic_sample = np.random.choice(chaotic, min(2, len(chaotic)), replace=False)
                non_chaotic_sample = np.random.choice(non_chaotic, min(2, len(non_chaotic)), replace=False)
                
                drive_forces = np.concatenate([chaotic_sample, non_chaotic_sample])
            else:
                # Just pick some random drive forces
                drive_forces = np.random.choice(df["num__drive_force_c"].unique(), 4, replace=False)
        
        if save_dir is not None:
            save_dir = Path(save_dir)
            save_dir.mkdir(parents=True, exist_ok=True)
        
        print(f"📈 Plotting phase space diagrams for {len(drive_forces)} drive forces...")
        ...
        for drive_force in drive_forces:
            ...
            plt.scatter(subset["num__theta"].iloc[0], subset["num__omega"].iloc[0], 
            ...
            if plot_3d and len(subset) > 100:
                ...
                points = ax.scatter(theta_t, theta_t_plus_delay, theta_t_plus_2delay, 
                                   c=np.arange(len(theta_t)), cmap='viridis', 
                                   s=10, alpha=0.8)
                ...
                ax.set_title(f"3D Phase Space: Drive Force c={drive_force:.4f} ({is_chaotic})")
...
def main():
    ...
    parser.add_argument(
        "--no-3d-plots", 
        action="store_true",
        help="Disable 3D phase space plots"
    )
    parser.add_argument(
        "--column", 
        type=str, 
    ...
    labeled_df = labeler.detect_chaos(df, column=args.column, use_correlation_dim=args.use_corr_dim)
    labeler.save_labeled_data(args.output)
    labeler.plot_phase_space(labeled_df, save_dir=args.plot_dir, plot_3d=not args.no_3d_plots)
    
    # Print summary of results
    if "is_chaotic" in labeled_df.columns:
        # Get unique drive forces and their chaotic status
        unique_forces = labeled_df[["num__drive_force_c", "is_chaotic"]].drop_duplicates()
        chaotic_count = unique_forces["is_chaotic"].sum()
        total_count = len(unique_forces)
        print(f"📊 Summary: {chaotic_count} out of {total_count} drive forces exhibit chaotic behavior.")
