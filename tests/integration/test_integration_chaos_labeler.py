...
class TestIntegrationChaosLabeler(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.simulation_data_path = cls.processed_dir / "chaotic_pendulum_simulations.csv"
    ...
    def _generate_test_data(cls):
        ...
        time_points = np.arange(0, cls.test_config["total_time"], cls.test_config["time_step"])
        num_points = len(time_points)
        
        # Use different drive forces to ensure a mix of chaotic and non-chaotic behavior
        drive_forces = [-1.5, -0.5, 0.8, 1.2, 1.5]  # Mix of values likely to produce different behaviors
        ...
        for c in drive_forces:
            ...
            if abs(c) > 1.0:  # More likely to be chaotic
                ...
                theta += 0.05 * np.sin(3.7 * time_points) + 0.02 * np.random.randn(num_points)
    ...
    def test_end_to_end_workflow(self):
        ...
        self.assertGreater(unique_forces["is_chaotic"].sum(), 0, "No chaotic cases detected")
        ...
        self.assertGreater(len(plot_2d_files), 0, "No 2D phase space plots were generated")
        self.assertGreater(len(plot_3d_files), 0, "No 3D phase space plots were generated")
    ...
    def test_main_function_integration(self):
        ...
        test_args = [
            "chaos_labeler.py",
            "--data", str(self.simulation_data_path),
            "--config", str(self.config_path),
            "--output", str(self.labeled_dir / "main_output.csv"),
            "--plot-dir", str(self.plots_dir / "main_plots"),
            "--use-corr-dim",
            "--column", "num__theta"
        ]
        ...
        self.assertGreater(len(plot_files), 0, "No plot files were generated by main function")
    ...
    def test_integration_with_different_columns(self):
        ...
        labeled_theta = labeler.detect_chaos(df, column="num__theta", use_correlation_dim=True)
        
        # Detect chaos using omega column
        labeled_omega = labeler.detect_chaos(df, column="num__omega", use_correlation_dim=True)
        
        # Compare results - they might differ as chaos can manifest differently in position vs velocity
        theta_chaos = labeled_theta[["num__drive_force_c", "is_chaotic"]].drop_duplicates()
        omega_chaos = labeled_omega[["num__drive_force_c", "is_chaotic"]].drop_duplicates()
        
        # We're not asserting they should be the same, just that both analyses completed successfully
        self.assertEqual(len(theta_chaos), len(omega_chaos), 
