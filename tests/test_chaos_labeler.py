...
class TestChaosLabeler(unittest.TestCase):
        ...
        cls.test_data = pd.DataFrame(
            {
                ...
                "num__drive_force_c": np.concatenate(
                    [np.full(500, 0.5), np.full(500, 1.2)]
                ),
            }
        )
    ...
    def test_init_and_load_config(self):
        ...
        self.assertEqual(
            self.labeler.config["chaos_detection"]["embedding_dimension"], 3
        )
    ...
    def test_calculate_correlation_dimension(self):
        ...
        with patch("numpy.sum", return_value=2):
            with self.assertLogs(level="WARNING") as cm:
                insufficient_corr_dim = self.labeler.calculate_correlation_dimension(
                    time_series
                )
    ...
    def test_detect_chaos(self):
        ...
        labeled_df = self.labeler.detect_chaos(
